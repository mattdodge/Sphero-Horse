...
import orbotix.robot.widgets.joystick.JoystickPuck;
import orbotix.robot.widgets.joystick.JoystickWheel;

/**
 * View that displays the Joystick, and handles the user's interactions with it.
 *
 * Created by Orbotix Inc.
 * @author Matt Dodge/Adam Williams
 */
public class SingleRobotJoystickView extends View implements Controller {
    ...
    public SingleRobotJoystickView(Context context, AttributeSet attrs) {
        super(context, attrs);
...
//                this.drive_control.driveJoyStick(drive_coord.x, drive_coord.y);
                ...
                int 	xPos = drive_coord.x - wheel_radius,
                		yPos = drive_coord.y - wheel_radius;
                
                float calcSpeed = speed * constrain(
                		Math.sqrt(Math.pow(xPos, 2) + Math.pow(yPos, 2)) / ((double) wheel_radius),
                		0, 1);
                
                float calcDir = (float) (Math.toDegrees(Math.atan2(-1 * xPos, yPos)) + 180.0);
                
//                Log.d("SpheroHorse", "Driving robot direction: "+calcDir+" with speed: "+calcSpeed + "....joystick at "+drive_coord.x + ","+drive_coord.y+" - "+wheel_radius);
//                Log.d("SpheroHorse", "Local point is "+local_point.x + ","+local_point.y);
//                Log.d("SpheroHorse", "My point is "+xPos + ","+yPos);
                
                RollCommand.sendCommand(robot, calcDir, calcSpeed);
...
//                this.drive_control.stopDriving();
    ...
    private float constrain(double theFloat, double min, double max) {
    	return (float) (Math.min(Math.max(theFloat, min), max));
    }
